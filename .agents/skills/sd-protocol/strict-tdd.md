# Strict TDD Module — Split Apply Lanes
...
- Do NOT write production code before the failing test exists; report `sdd-apply-unit-tests` as the required RED owner to the coordinator. The current executor must not launch that lane.
