...
This library depends on the project locator and binding defined by each overlay; it does not ship a binding itself. The mis-proyectos overlay's binding lives at `.agents/skills/projectctl-requirements/references/tasks/binding.md` (block delimited by `<!-- task-flow-binding:start -->` / `<!-- task-flow-binding:end -->`); another overlay may declare any other binding path or none.
