...
This section is the **single source of truth** for the boilerplate that is identical across every executor lane (apply, verify, tasks). A lane MUST reference this section instead of restating any of these blocks inline. Concrete values are always resolved from the versioned `WorkflowRuntimeContext`; nothing here is bound to a specific project overlay.
...
|---|---|---|
| `workflow_context_ref` | Coordinator injection (validated `WorkflowRuntimeContextV1` or `WorkflowRuntimeContextV2`) | Yes |
| `artifact_refs` | Lane-owned file(s)/content resolved from `WorkflowRuntimeContext.task_ref` and the assigned unit | Yes |
...
A lane MUST refuse to invent path, branch, status, phase, mirror adapter, mirror key, close rule, or topic-pattern value. Every concrete value comes from the versioned `WorkflowRuntimeContext` and from its bounded lane registry.

When the snapshot is V2, the same refuse-to-invent rule applies to `mode_context.rdd`, authority lineage/revision/target/task-revision bindings, transition kind, operation, lane and tokens. V1 is disabled-only and MUST NOT authorize an RDD lane. Executors never use `rdd-report` or their own envelope as authority evidence.

### F.2 Raw-input prohibition (universal)

A lane MUST NOT read `source.config_path`, `source.binding_path`, or `state_model_ref.path`. All workflow policy is consumed through the bounded accessors of the versioned `WorkflowRuntimeContext`; the lane never opens the raw binding or locator.
