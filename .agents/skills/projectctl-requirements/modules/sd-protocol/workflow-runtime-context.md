...
> **Single source of truth** for the formal shapes of `WorkflowRuntimeContextV1` and `WorkflowRuntimeContextV2` and the rules the coordinator MUST follow to resolve, validate, and inject them. Consumers (`coordinador`, every `sdd-*` lane, `sd-protocol`) read this file to learn the contract; no other file restates the schema. This contract is binding-mechanism-level: concrete values are never hardcoded here and always arrive through the binding the locator points to.

The formal contract in this file is referenced by:

- The consumer-side resolver in `coordinador/SKILL.md` `## WorkflowRuntimeContext resolution`.
...
A coordinator that consumes a supported versioned binding satisfying §3 needs an injectable runtime context whose fields are bounded, strict-optional, and free of any local literal; whose source identity is traceable from the locator and binding; and whose failure handling is **closed** (missing/mismatched/ambiguous inputs return `blocked` and preserve state). Any lane that depends on its shape MUST reference this file — never restate it.
...
12. **Hooks are universal.** Gate hooks (`envelope_gate`, `transition_gate`, `app_map_close_gate`, `revision_gate`, `evidence`, `hard_gate`, `rdd_delivery_gate`) are configurable evaluators declared by the binding; their identifiers are read from the binding's `gates` registry, never invented.
...
16. **RDD authority is external and opaque.** V2 transports authority-issued lineage, revision, target and task-revision bindings without deriving, repairing or approving them. A task index, frontmatter value, UI state, phase artifact or `rdd-report` is never authority evidence.
17. **Uncertain mutation requires status.** A timeout, lost response, stale CAS, partial result, `mutation_outcome: unknown`, or `replayability: status_required` suppresses blind retry and transition; the next operation is an authority status query followed by reconciliation.
...
interface WorkflowModeContextEntryV2 {
...
interface RddAuthorityEvidenceV2 {
  readonly source: "provider-status" | "provider-operation" | "provider-gate";
...
interface RddAuthorityBindingV2 {
  readonly lineage_id: string;
  readonly revision: string;
  readonly target_identity: string;
  readonly task_revision: string;
...
type RddNextTransitionV2 =
  | {
      readonly kind: "execute";
      readonly operation: string;
      readonly arguments: readonly { readonly token: string }[];
      readonly authority_binding: RddAuthorityBindingV2;
    }
  | {
      readonly kind: "collect";
      readonly lane_id: string;
      readonly inputs: readonly Record<string, unknown>[];
      readonly capture_tokens: readonly string[];
      readonly authority_binding: RddAuthorityBindingV2;
    }
  | {
      readonly kind: "stop";
...
interface WorkflowRuntimeContextV2 extends Omit<WorkflowRuntimeContextV1, "contract_version" | "mode_context"> {
  ...
  readonly mode_context: {
    ...
    readonly rdd: WorkflowModeContextEntryV2 & {
      readonly selected: "disabled" | "receipt-driven";
      readonly default: "disabled";
      readonly allowed: readonly ["disabled", "receipt-driven"];
    };
    readonly resolved_mechanism_skill_paths: {
      readonly review: readonly string[];
      readonly delivery: readonly string[];
      readonly rdd: readonly string[];
    };
  };
  readonly rdd_authority_context: {
    readonly evidence: RddAuthorityEvidenceV2 | null;
    readonly allowed_transition_kinds: readonly ["execute", "collect", "stop"];
    readonly next_transition: RddNextTransitionV2 | null;
    readonly status_required: boolean;
  };
  readonly gate_context: WorkflowRuntimeContextV1["gate_context"] & {
    /** Bounded configuration only; the mutable record is never handed to a lane. */
    readonly environment_deferral: {
      readonly record_owner: "coordinator";
...
type WorkflowRuntimeContext = WorkflowRuntimeContextV1 | WorkflowRuntimeContextV2;

function isWorkflowRuntimeContextV2(
  context: WorkflowRuntimeContext,
): context is WorkflowRuntimeContextV2 {
  return context.contract_version === 2;
}
```

### 4.0 Version discrimination and legacy adapter

- Consumers MUST discriminate the union only with `isWorkflowRuntimeContextV2` / `contract_version`; field-presence probing is not a version guard.
- A V1 reader MAY expose a read-only compatibility view whose `mode_context.rdd.selected` is exactly `disabled`, with no RDD mechanism paths. V1 MUST NOT authorize any RDD lane, operation, gate, transition or delivery bypass.
- Producers for a V2 binding emit V2 only. Missing, unknown or partial V2 RDD mode configuration is not adapted to V1 and returns `mode_config_invalid` or `mode_selection_invalid`.
- There are no aliases or fallbacks between versions, mode values, transition kinds, operation ids, lane ids, evidence ids or authority tokens.
...
- `workflow_context_ref` in every lane means an immutable reference to one fully materialized `WorkflowRuntimeContext` snapshot.
...
- Mode selection reads only the immutable `mode_context` after materialization. For each mode, `selected` is the explicit active-task frontmatter value when present and otherwise `default`; both MUST belong to `allowed`. In V2 this includes `mode_context.rdd`. `mechanism_skill_ids` contains only logical ids from the selected binding configuration. Exact registry-validated physical paths are isolated in `resolved_mechanism_skill_paths` and are orchestration mechanisms, never lane skills or additions to `lane_context.skill_paths`.
- RDD guards read only `mode_context.rdd.selected` plus current `rdd_authority_context.evidence`. Authority evidence is current only when its opaque `lineage_id`, `revision`, `target_identity`, and `task_revision` all exactly match the provider response and active task snapshot. Missing, partial, stale or mismatched evidence blocks; no value is reconstructed from `rdd-report`, frontmatter, UI, an executor envelope or prior chat.
- V2 environmental-deferral evaluation reads only `gate_context.environment_deferral` plus coordinator-read canonical index evidence. The accessor contains gate/configuration identities, never the mutable pending record. Its allowed-method set is closed, non-empty, duplicate-free and validated against referenced gate configuration. Lanes receive no write/evaluate accessor for recording, evidence removal, revocation or completion.
- `rdd_authority_context.next_transition` is a strict discriminated union. `execute` requires an exact operation and authority-issued argument tokens; `collect` requires one exact lane, immutable inputs and capture tokens; `stop` requires an exact reason. Every branch carries all four authority bindings. Unknown keys do not supply missing required fields, and prose/legacy action fields are never parsed as fallback routing.
...
- `environment_deferral_config_invalid` — V2 environmental-deferral configuration is absent, partial, empty, duplicated, inconsistent across mode/gates, names an unknown method, or does not declare coordinator ownership and fail-closed policies. Resolution blocks before context materialization.
- `environment_deferral_record_invalid` — an active record or audit is malformed, stale, revision-mismatched, empty in an active position, inconsistent with the configured method lifecycle, or cannot be reconciled exactly. Preserve the record and task position; never normalize or delete it.
- `environment_deferral_evidence_invalid` — method-completion evidence is missing, self-reported, uncertain, stale, applies to another verification revision, or does not prove exactly one pending method. Preserve every pending method and block.
- `rdd_transition_unknown` — V2 authority status declares a transition kind, operation, lane, token shape or identifier outside the exact binding/provider allowlist. No alias or fallback is attempted.
- `rdd_authority_evidence_invalid` — authority evidence is missing/partial, comes from a non-authoritative report, or its lineage, revision, target identity or task revision does not exactly bind the requested guard.
- `rdd_authority_response_partial` — a required result set or authority response is incomplete; partial reviews never satisfy completion or approval.
- `rdd_required_but_globally_disabled` — the task selected receipt-driven mode while the authority kill switch reports disabled; the current position is preserved.
- `rdd_stale_revision` — authority CAS rejects the supplied revision; no result or task state is overwritten and status reconciliation is mandatory.
- `rdd_status_required` — mutation outcome is uncertain, response was lost, or replayability requires status; blind retry and advancement are suppressed until fresh authority status is reconciled.
- `rdd_receipt_not_applicable` — receipt is missing, escalated, stale, drifted or bound to another lineage/revision/target/task revision; reports cannot satisfy this check.
- `rdd_authority_ambiguous_or_corrupt` — authority status is unknown, malformed, corrupt or cannot establish one exact live revision; managed fail-closed recovery is required.
...
1. The bounded `WorkflowRuntimeContext` V1-or-V2 snapshot/reference (no less, no more).
...
- **Project binding files** outside `sd-protocol/`: a project's binding (e.g. `projectctl-requirements/references/tasks/binding.md`) declares the concrete values this contract keeps parametric.
...
The current producer shape is `WorkflowRuntimeContextV2`; `WorkflowRuntimeContextV1` remains a disabled-only legacy reader shape. Compatible additive V2 accessors bump the implementation contract skill's MINOR version; incompatible field removal or type narrowing requires a new numbered context contract and a MAJOR skill bump. Adding a closed fail-safe reason for a new accessor is compatible when existing successful inputs retain their behavior. The implementation contract skill's `metadata.version` is authoritative. The binding's `binding_id` / `binding_version` remain independent identities.

The primary-index/phase-artifact split (§4.3), envelope summary+ref fields (§4.4), and knowledge-role mirror remain shared by both versions. RDD mode and authority-bound routing belong only to V2; they MUST NOT be backported as V1 authorization.
