...
The exact gate matrix per `apply_lane` is defined once in `.agents/skills/projectctl-requirements/modules/sd-protocol/apply-work-unit-schema.md §5`; this skill references it and never restates it divergently.

## Required Inputs

Universal executor inputs are defined in `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md` §F.1. This lane additionally requires:
...
Follow **Section A** from `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md`. For `code-high`, the coordinator injects every required surface policy through `lane_context.surface_skill_paths`; this lane reads only the injected `lane_context` paths and blocks with `skill_resolution_missing` instead of discovering or recovering any missing path.
...
- If Strict TDD is active (`tdd_mode: strict` on the assigned work unit), load `.agents/skills/projectctl-requirements/modules/sd-protocol/strict-tdd.md` and follow it instead of the standard workflow.
...
- Use repo-local module path `.agents/skills/projectctl-requirements/modules/sd-protocol/strict-tdd.md` when Strict TDD is active.
- Never address a retired alias — see `sdd-phase-common.md` §F.3.
