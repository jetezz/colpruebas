...
Universal Required Inputs are defined in `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md §F.1`; the rows above declare only the lane-specific additions (including `browser_runtime_preconditions`). The lane MUST refuse to invent `BASE_URL`, commands, credentials, runtime ownership, branch, status, phase, mirror, or topic values. Raw-input prohibition is defined in `sdd-phase-common.md §F.2`.

## Authorization Gate

The universal authorization steps (validated resolver context; this lane id present in `lane_context.registry` + `allowed_lanes`; the `explore-pwcli` key present in `artifact_context.phase_artifacts.artifact_keys`) are defined in `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md §F`. Lane-specific addition:
...
See `.agents/skills/projectctl-requirements/modules/sd-protocol/explorer-rules.md` §Common Hard Rules and §Lane-specific decision gates. Lane-specific rules for this lane (browser preconditions, exploration-only `playwright-cli`, no verification closure, no code/research broadening): `explorer-rules.md` §Lane-specific Hard Rules > sdd-explore-pwcli.
...
See `.agents/skills/projectctl-requirements/modules/sd-protocol/explorer-rules.md` §Lane-specific decision gates. Routing references address the split exploration lanes by their exact identifiers as declared in `WorkflowRuntimeContextV1.lane_context.registry`.
...
- `.agents/skills/projectctl-requirements/modules/sd-protocol/workflow-runtime-context.md`
- `.agents/skills/projectctl-requirements/modules/sd-protocol/explorer-rules.md`
- The active binding block in `references/tasks/binding.md` §`task-flow-binding`
