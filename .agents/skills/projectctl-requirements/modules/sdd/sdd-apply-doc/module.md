...
- If Strict TDD is active for the assigned work unit, load `.agents/skills/projectctl-requirements/modules/sd-protocol/strict-tdd.md` and note that RED tests are owned by `sdd-apply-unit-tests`; this lane handles documentation only.
...
The full Project Rules block is injected by the coordinator from `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md` §E on every delegation. Do not inline the block here — single source of truth in the protocol.
