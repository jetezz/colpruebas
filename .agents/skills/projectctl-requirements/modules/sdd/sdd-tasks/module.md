...
Universal Required Inputs are defined in `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md §F.1`; the rows above declare only the planning-specific additions. Raw-input prohibition is defined in `sdd-phase-common.md §F.2`. Retired aliases are governed by `sdd-phase-common.md §F.3`; this lane never re-enumerates the retired-alias set.

## Authorization Gate

The universal authorization steps (validated resolver context; this lane id present in `lane_context.registry` + `allowed_lanes`; the `tasks` key present in `artifact_context.phase_artifacts.artifact_keys`) are defined in `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md §F` and enforced by the resolver. Planning-specific addition:
...
> Execution and persistence follow `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md §F.4` and, under the index-primary variant, `§F.5`. The apply work-unit schema (13 columns, 4 contract fields, complexity evaluation, parallel-safety, pre-implementation gate, verification mapping) is defined in `.agents/skills/projectctl-requirements/modules/sd-protocol/apply-work-unit-schema.md`. This lane populates rows conforming to that schema.
...
Follow **Section A** from `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md`.
...
Routing when a verification lane finds missing coverage is defined in `.agents/skills/projectctl-requirements/modules/sd-protocol/apply-work-unit-schema.md §6` (verification mapping); it is verification-lane behaviour, not a planning responsibility.
