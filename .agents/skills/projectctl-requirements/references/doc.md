...
Esta skill es el **estándar canónico de compatibilidad `/projectctl`** y este archivo cubre PCT-83..88. Cada requisito cita fuentes relacionadas en formato estructurado y declara `Cumple: PCT-XX` + `last-verified: YYYY-MM-DD`.
...
El panel Doc MUST listar las 6 secciones canónicas MUST por bundle (`URL`, `Tab`, `Objetivo`, `Criterios`, `Diagrama Mermaid`, `Sources`) como checklist navegable, con marcador obligatorio por sección y enlace al estándar integrado `.agents/skills/projectctl-requirements/references/standard.md` como policy de origen. Cada bundle de `docs/app-map/` está obligado a declarar estas 6 secciones; `Sources` cita fuentes y no las copia, y los diagramas Mermaid viven como `${bundle}.mmd` siblings.
...
El panel Doc MUST explicar el contrato `criteria[]` inline en frontmatter YAML: estructura `{id, title, functional, coverage, type}` con IDs prefijados por bundle (`PCT-*` para `views/projectctl/`, `PRJ-*` para `views/project-workspace/`, `TST-*` para test-system, `AC-*` cross-cutting, `DSH-*` para `views/dashboard/`, `TNL-*` para tunnel, `LGN-*` para `views/login/`, `MDL-*` para `views/models/`) y métodos/cobertura permitidos (`Unit`, `PW-CLI`, `PW-AUTO`, `Manual` × `covered`, `partial`, `missing`, `not-applicable`). Estados funcionales permitidos: `implemented | partial | missing | not-applicable`. `type` es el campo obligatorio de clasificación del criterio con enum cerrado de 9 valores: `ui | functionality | a11y | backend | data | integration | security | performance | tooling` (T-1; árbol de clasificación y mapeo de verificación por defecto en `references/standard.md` §1). El contrato es **bidireccional** (R-B): todo criterio declarado en `docs/app-map/**` debe tener evidencia en código o tests, o una justificación explícita (not-applicable / Manual / documental); la dirección doc⇒código se audita en close por bundle (procedimiento "Auditoría de criterios" en `references/standard.md` §1) y NO es un hard gate de lint.
...
El panel Doc MUST renderizar un bloque `<aside data-testid="projectctl-tab-sources-doc">` con la tabla de sources (skill path + bundle path + URL API + test path) para que un agente o humano sepa de dónde sale cada regla. Cada tab (`cli | tareas | agentes | doc | criterios | test | entorno | estructura`) MUST tener su propio aside `<aside data-testid="projectctl-tab-sources-<tab>">` con kebab-case `<tab>` perteneciente a ese conjunto canónico.
...
| ID | Criterio | Estado |
|---|---|---|
| **PCT-83** | `/projectctl?tab=doc` MUST renderizar panel Doc listando las 6 secciones MUST + contrato `criteria[]` inline + prefix discipline + SoT única + eliminación de los archivos quality-plan.md y quality-status.md (legacy per TST-03/TST-12) | `añadir` |
| **PCT-84** | Panel Doc MUST listar las 6 secciones canónicas MUST (`URL`, `Tab`, `Objetivo`, `Criterios`, `Diagrama Mermaid`, `Sources`) como checklist navegable con marcador obligatorio | `añadir` |
...
**Bundles documentales relacionados**: `docs/app-map/views/projectctl/features/doc.md` + `docs/app-map/views/projectctl/features/doc.mmd` + mapping en `docs/app-map/views/projectctl/index.md` + registro de las 8 tabs en `docs/app-map/navigation.yaml`.
