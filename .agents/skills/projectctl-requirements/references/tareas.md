...
# Task flow binding — `projectctl-requirements` v8.0.0
...
> **v8.0.0 persistence model (breaking).** The compact coordination index at `taskReadme/<task_id>-<task_slug>.md` and its referenced phase artifacts under `taskReadme/<task_id>-<task_slug>/<artifact>.md` are the complete canonical persistence and recovery sources. This overlay configures no mirrors. Optional general support tools, including memory tools, are never SDD evidence, recovery backends, closure prerequisites, or sources of truth. Generic mirror support remains available only for another binding that explicitly configures mirrors.
