...
purpose: binding operational contract for task creation, five-phase execution, evidence, RDD, and delivery
sot_policy: canonical-standard
version: 10.0.0
last_full_regen: 2026-09-08
binding_id: projectctl-requirements.task-flow
model_version: 2
---

# Task flow binding — `projectctl-requirements` v10.0.0

## Purpose

This file is the **single operational binding** for agents that create, implement, verify, document, and deliver SDD tasks on the `mis-proyectos` overlay. Its sole purpose is to host the machine-consumable binding contract that the coordinator module, `sd-protocol` and the binding-declared workflow lanes read through a `WorkflowRuntimeContextV2` resolved from this file. A V1 reader is legacy-only and resolves RDD exclusively as `disabled`; it never authorizes an RDD lane.

The binding resolves the entire local task workflow — identity, persistence, the five-phase state machine, action controls, lane matrix, gates, delivery, and active-source allow-list — without requiring a second normative catalog. The prose before and after the delimited block is **context only**.

> **v10.0.0 persistence and helper contract.** The compact coordination index and referenced phase artifacts are the complete canonical persistence and recovery sources. This overlay configures no mirrors; `rdd-report` is a non-authoritative projection. The binding also declares `task_skill_selection`: task snapshots select project-installed helper skills by stable `metadata.id`, while every execution preserves the mandatory lane → configured surfaces → optional helpers order. The professional Tasks CLI inputs and explicit selection modes are part of the same portable contract.
...
| Path (repo-relative) | `.agents/skills/projectctl-requirements/references/tasks/binding.md` |
...
- Persistence implementation: `.agents/skills/projectctl-requirements/modules/sd-protocol/persistence-contract.md` and `.agents/skills/projectctl-requirements/modules/sd-protocol/sdd-phase-common.md` — universal protocol mechanisms.
...
```json
{
  ...
  "binding_version": "10.0.0",
  "model_version": 2,
  "task_document_compatibility": {
    "readable_binding_versions": ["8.0.0", "9.0.0"],
    "legacy_v1": {
      "missing_rdd_mode": "disabled",
      "allowed_rdd_mode": "disabled",
      "authorize_rdd_lanes": false,
      "rewrite_on_read": false
    },
    "unknown_rdd_mode": "block",
    "mixed_package_or_binding_versions": "block"
  },
  "task": {
    "required_inputs": [
      "task_name",
  ...
  "modes": {
    ...
    "rdd_mode": {
      "default": "disabled",
      "allowed": ["disabled", "receipt-driven"],
      "skills": ["rdd-phase", "rdd-review", "rdd-correct", "rdd-validate"],
      "legacy_v1_read": "disabled-only",
      "unknown_value": "block",
      "description": "Opt-in task-scoped Receipt-Driven Development. Disabled preserves ordinary delivery; receipt-driven requires authority-backed Phase 5 and live delivery gates."
    },
    "environment_deferral": {
      "record_owner": "coordinator",
      "recording_location": "task.index.coordinator_owned_sections",
  ...
  "phases": [
    ...
    },
    {
      ...
      "transitions": [
        ...
        {"from": "p4_reviewing", "to": "p4_complete", "guard": "documentation_gate_passed", "blocking_gates": ["pending_environment_close_block"]},
        {"from": "p4_complete", "to": "final_commit_pending", "guard": "rdd_mode_disabled", "blocking_gates": ["pending_environment_close_block"]},
        {"from": "p4_complete", "to": "p5_started", "guard": "rdd_mode_receipt_driven", "blocking_gates": ["pending_environment_close_block"]}
      ]
    },
    {
      "id": "fase_5_rdd",
      ...
      "transitions": [
        {"from": "p5_started", "to": "p5_reviewing", "guard": "rdd_authority_started"},
        {"from": "p5_reviewing", "to": "p5_correction_required", "guard": "rdd_correction_required"},
        {"from": "p5_reviewing", "to": "p5_validating", "guard": "rdd_review_results_complete"},
        {"from": "p5_correction_required", "to": "p5_validating", "guard": "rdd_correction_validated"},
        {"from": "p5_validating", "to": "p5_complete", "guard": "rdd_receipt_approved"},
        {"from": "p5_reviewing", "to": "p5_escalated", "guard": "rdd_escalated"},
        {"from": "p5_correction_required", "to": "p5_escalated", "guard": "rdd_escalated"},
        {"from": "p5_validating", "to": "p5_escalated", "guard": "rdd_escalated"},
        {"from": "p5_escalated", "to": "p5_started", "guard": "rdd_recovery_authorized"},
        {"from": "p5_complete", "to": "final_commit_pending", "guard": "rdd_precommit_gate_allowed"}
      ]
    }
  ],
  "controls": [
      ...
      "transitions": [
        {
          ...
          "blocking_gates": ["pending_environment_close_block"]
        },
        {
  ...
  "lanes": {
    ...
    "rdd-review-risk": {
      "skill": "rdd-review",
      "role": "rdd-review",
      "artifact_class": "rdd_review_result",
      "owner_phase": "fase_5_rdd"
    },
    "rdd-review-readability": {
      "skill": "rdd-review",
      "role": "rdd-review",
    ...
    "rdd-refuter": {
      "skill": "rdd-review",
      "role": "rdd-review",
      "artifact_class": "rdd_refutation_result",
      "owner_phase": "fase_5_rdd"
    },
    "rdd-correct": {
      "skill": "rdd-correct",
      "role": "rdd-correction",
      "artifact_class": "rdd_correction_evidence",
      "owner_phase": "fase_5_rdd"
    },
    "rdd-validate": {
      "skill": "rdd-validate",
      "role": "rdd-validation",
      "artifact_class": "rdd_validation_evidence",
      "owner_phase": "fase_5_rdd"
    }
  },
  "gates": {
    ...
    "pending_environment_close_block": {
      ...
      "configuration": {
        ...
        "blocked_targets": ["p4_complete", "final_commit_pending", "final_push_pending", "final_pr_pending", "p5_started", "done"],
        "non_overrides": ["rdd_receipt", "human_approval", "rdd_report", "ui_action"]
      }
    },
    "rdd_mode_disabled": {
      "evaluator": "transition_gate",
      "required_evidence": ["mode_context.rdd.selected_disabled"],
      "failure": "retain_phase_and_state"
    },
    "rdd_mode_receipt_driven": {
      "evaluator": "transition_gate",
      "required_evidence": ["mode_context.rdd.selected_receipt_driven", "rdd_authority_available_and_managed"],
      "failure": "retain_phase_and_state"
    },
    "rdd_authority_started": {
      "evaluator": "hard_gate",
      "required_evidence": ["rdd_provider_result_current", "rdd_lineage_id", "rdd_authority_revision", "rdd_target_identity", "task_revision"],
      "failure": "retain_phase_and_state"
    },
    "rdd_correction_required": {
      "evaluator": "hard_gate",
      "required_evidence": ["rdd_provider_result_current", "rdd_correction_request", "rdd_lineage_id", "rdd_authority_revision", "rdd_target_identity", "task_revision"],
      "failure": "retain_phase_and_state"
    },
    "rdd_review_results_complete": {
      "evaluator": "hard_gate",
      "required_evidence": ["rdd_provider_result_current", "rdd_required_results_complete", "rdd_lineage_id", "rdd_authority_revision", "rdd_target_identity", "task_revision"],
      "failure": "retain_phase_and_state"
    },
    "rdd_correction_validated": {
      "evaluator": "hard_gate",
      "required_evidence": ["rdd_provider_result_current", "rdd_post_apply_allowed", "rdd_lineage_id", "rdd_authority_revision", "rdd_target_identity", "task_revision"],
      "failure": "retain_phase_and_state"
    },
    "rdd_receipt_approved": {
      "evaluator": "hard_gate",
      "required_evidence": ["rdd_provider_result_current", "rdd_receipt_outcome_approved", "rdd_receipt_applies_to_live_target", "rdd_lineage_id", "rdd_authority_revision", "rdd_target_identity", "task_revision"],
      "failure": "retain_phase_and_state"
    },
    "rdd_escalated": {
      "evaluator": "hard_gate",
      "required_evidence": ["rdd_provider_result_current", "rdd_escalation_recorded", "rdd_lineage_id", "rdd_authority_revision", "rdd_target_identity", "task_revision"],
      "failure": "retain_phase_and_state"
    },
    "rdd_recovery_authorized": {
      "evaluator": "hard_gate",
      "required_evidence": ["explicit_rdd_recovery_authorization", "rdd_provider_recovery_transition_current", "rdd_lineage_id", "rdd_authority_revision", "rdd_target_identity", "task_revision"],
      "failure": "retain_phase_and_state"
    },
    "rdd_precommit_gate_allowed": {
      "evaluator": "hard_gate",
      "required_evidence": ["rdd_pre_commit_allowed", "rdd_receipt_applies_to_live_target", "rdd_lineage_id", "rdd_authority_revision", "rdd_target_identity", "task_revision"],
      "failure": "retain_phase_and_state"
    },
    "post-apply": {
      "evaluator": "rdd_delivery_gate",
      "required_evidence": ["authority_gate_result_current", "allowed_true", "live_target_identity_match"],
      "failure": "retain_phase_and_state"
    },
    "pre-commit": {
      "evaluator": "rdd_delivery_gate",
      "required_evidence": ["authority_gate_result_current", "allowed_true", "approved_receipt_applicable", "live_target_identity_match"],
      "failure": "retain_final_commit_pending"
    },
    "pre-push": {
      "evaluator": "rdd_delivery_gate",
      "required_evidence": ["authority_gate_result_current", "allowed_true", "approved_receipt_applicable", "live_target_identity_match"],
      "failure": "retain_final_push_pending"
    },
    "pre-pr": {
      "evaluator": "rdd_delivery_gate",
      "required_evidence": ["authority_gate_result_current", "allowed_true", "approved_receipt_applicable", "delivery_target_branch_used_as_base_ref"],
      "failure": "retain_final_pr_pending"
    },
    "release": {
      "evaluator": "rdd_delivery_gate",
      "required_evidence": ["authority_gate_result_current", "allowed_true", "approved_receipt_applicable", "live_target_identity_match"],
  ...
  "delivery": {
    ...
    "close_blocking_gates": ["pending_environment_close_block"],
    "rdd": {
      "mode_source": "modes.rdd_mode",
      "disabled": {
        "invoke_authority": false,
        "required_gates": []
      },
      "receipt-driven": {
        "invoke_authority": true,
        "required_gates": ["post-apply", "pre-commit", "pre-push", "pre-pr", "release"],
        "gate_behavior": "read-only-live-candidate-validation",
        "pre_pr_base_ref_source": "delivery.target_branch",
        "deny_or_error": "preserve_pending_control_and_block_action"
      }
    },
    "delivery_modes": {
      "single-pr": {
  ...
  "active_sources": {
    "include": [
      ...
      ".agents/skills/projectctl-requirements/references/tasks/binding.md",
