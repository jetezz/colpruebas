...
description: "Trigger: projectctl rules, project onboarding, requirements checklist, docs governance, testing policy, runtime policy, task flow, projectctl operation. Cross-repo portable standard for `/projectctl` compatibility (cli | tareas | agentes | doc | criterios | test | entorno | estructura)."
metadata:
...
Estándar cross-repo instalable y **canónico para compatibilidad `/projectctl`** (`cli | tareas | agentes | doc | test | entorno`). Centraliza los checklists y las reglas operativas mínimas que antes vivían repartidas en `docs-governance`, `testing-policy`, `ops-runtime-policy` y `projectctl-operator`, manteniendo citas a fuentes de código/docs como contexto/evidencia en formato `<aside data-skill-sot>`.
...
> Estándar cross-repo instalable y canónico de compatibilidad `/projectctl` (`cli | doc | test | entorno | tareas | estructura`), con requisitos, reglas documentales, reglas de testing, runtime gestionado, flujo de tareas y operación segura del CLI en un solo paquete.

**Lo que incluye**:

- Requisitos de las 8 tabs `cli | tareas | agentes | doc | criterios | test | entorno | estructura`.
...
- No es un catálogo del CLI. El catálogo visible actual lo genera `sandbox/src/lib/projectctl-registry.ts` y se proyecta a `frontend/src/views/projectctl/data/cli-catalog.generated.ts`; el App Map generado conserva la trazabilidad histórica PCT-01..PCT-78. Esta skill solo agrega el **índice de reglas que un proyecto debe cumplir** para que ese CLI se renderice en `/projectctl`.
...
- **NO** usar como documentation editorial general: las decisiones D-1..D-21 que rigen esta skill viven en [`references/decisions.md`](references/decisions.md) (binding canónico del paquete). El lineage extendido permanece como contexto histórico, no como fuente normativa.

## Sections (resumen de las 8 tabs)

Esta skill cubre las 8 tabs internas de `/projectctl` que un proyecto nuevo debe entender para ser compatible. Cada tab tiene su archivo `references/<tab>.md` con requisitos testeables, trazabilidad y `last-verified: YYYY-MM-DD`:

| Tab | URL | Qué cubre | Archivo de detalle |
|---|---|---|---|
| **cli** | `/projectctl?tab=cli` (default) | Catálogo generado registry-only de los comandos visibles, agrupados por familia, con copia al portapapeles y filtro `name`; el count vigente y las familias se leen del output generado. | [`references/cli.md`](references/cli.md) |
| **doc** | `/projectctl?tab=doc` | Reglas documentales que un proyecto debe cumplir para que su `docs/app-map/` se renderice bien. 6 secciones MUST por bundle, contrato `criteria[]` inline, prefix discipline, SoT única, eliminación de archivos legacy. Cubre PCT-83..88. | [`references/doc.md`](references/doc.md) |
| **criterios** | `/projectctl?tab=criterios` | Índice derivado de los criterios de aceptación: autoridad inline del bundle, distinción de enums, ledger separado y trazabilidad de sources. | [`references/criterios.md`](references/criterios.md) |
| **test** | `/projectctl?tab=test` | Reglas del sistema de testing: AC mandatorio (`// @ac <ID>`), runner unificado, runs persistidos con coverage `pending`/no aceptada y auto-writeback diferido, gate `bun run test:check`, layout canónico. Cubre PCT-89..94. | `references/test.md` (scope de WU-SKILL-2) |
| **tareas** | `/projectctl?tab=tareas` | Proyección informativa del binding vigente; no es una superficie mutable ni una segunda autoridad. | [`references/tareas.md`](references/tareas.md) + [`references/tasks/binding.md`](references/tasks/binding.md) |
| **agentes** | `/projectctl?tab=agentes` | Mapa derivado de lanes/roles y módulos internos; no republica el catálogo operativo del binding. | [`MAP.md`](MAP.md) + módulos internos |
| **entorno** | `/projectctl?tab=entorno` | Reglas portables para que un proyecto gestionado arranque y sea publicable: overlays canónicos, puertos explícitos, red/alias edge abstractos y sandbox sin Docker. Los nombres, secretos y topología del operador se documentan en los docs locales del repo. Cubre PCT-95..100. | `references/entorno.md` (scope de WU-SKILL-2) |
| **estructura** | `/projectctl?tab=estructura` | Estructura top-level 7+3, capabilities y mapping canónico `criteria[].type` → carpeta (`mapping v1.0.0`), con aliases, diagnósticos y rollout warn→error. | [`references/estructura.md`](references/estructura.md) |
...
- `references/doc.md` — requisitos tab Doc (PCT-83..88) — **presente** (WU-SKILL-1).
  - `references/criterios.md` — índice derivado de criterios (PCT-169..175), cita-no-copia y sin autoridad propia.
- `references/tasks/binding.md` — **única SoT normativa** del flujo SDD. Contiene el bloque delimitado `task-flow-binding` (`TaskFlowBindingV2`, v10.0.0, model 2). Otras referencias solo lo citan y no republican sus machine values.
...
- `references/estructura.md` — estructura estándar 7+3 y mapping `criteria[].type` → carpeta (`mapping v1.0.0`), con aliases, diagnósticos, capabilities y SemVer.
...
5. **Binding único de tareas**. El flujo operativo completo vive en `references/tasks/binding.md`; otras superficies solo lo referencian o generan una representación informativa, sin duplicar su catálogo.
...
**Status**: estándar canónico de compatibilidad `/projectctl`, package v13.1.0, binding v10.0.0, scope `repo` (no global), `sot_policy: canonical-standard`, **last-verified: 2026-09-20**. El bloque `task-flow-binding` (`TaskFlowBindingV2`) dentro de `references/tasks/binding.md` es la única SoT normativa; V1 solo se lee como legacy disabled-only y cualquier paquete/binding mezclado bloquea y requiere reemplazo `copy-tree-no-mods` completo.
