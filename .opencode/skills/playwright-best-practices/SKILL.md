...
## Project-Specific Configuration (OpenCode Agent)
...
| **Workers** | `50%` (optimal: 6 on 12-core) |

### 7-Project Architecture

| Project | Purpose | Workers |
|---|---|---|
| `setup` | Auth setup (global-setup.ts) | 1 |
| `api` | API-only tests, no browser | default |
| `auth-flow` | Login/logout (serial) | 1 |
| `chromium-readonly` | Read-only UI (fullyParallel) | default |
| `chromium` | Standard mutating tests | default |
| `chromium-stateful` | CRUD / order-dependent (serial) | 1 |
| `chromium-infra` | Infra tests (tunnel, deploy) | 1 |

### Mandatory Rules

- **Selectors**: Use `.project-card`, `.modal-footer button`, `getByRole` — NEVER `main.filter({ hasText })`
- **Cleanup**: Every CREATE must have a matching DELETE in `finally {}` or after assertions
- **Worker isolation**: Use `workerResourceName(testInfo, prefix)` and `workerTimestampName(prefix)` from `helpers/worker-resources.ts`
- **Auth tokens**: Use `getApiToken(page)` from `helpers/api-token.ts` — NEVER extract tokens manually
- **Wait strategy**: `expect(locator).toBeVisible()` — NEVER `waitForTimeout()`
...
1. **Run tests**: `cd test && bun run playwright test --reporter=list`
2. **If tests fail**:
   - Review error output and trace (`cd test && bun run playwright show-trace`)
   - Fix locators, waits, or assertions
   - Re-run tests
3. **Only proceed when all tests pass**
4. **Run multiple times** for critical tests: `cd test && bun run playwright test --repeat-each=5`
5. **Run specific project**: `cd test && bun run playwright test --project=chromium-stateful`
