...
- Name: test-tab.spec.ts >> Tab Test funcional — @test-tab (PWT-12) >> abrir modal Reset coverage muestra texto verbatim del contrato
- Location: tests/front/tests/test-tab.spec.ts:72:3
...
Call log:
  - waiting for locator('[data-testid="test-tab-quickrun-reset-coverage"]')
...
> 82  |       .click();
  ...
  128 |       .click();
  129 | 
  130 |     await manualButtons.first().click();
  131 |     const dialog = page.locator('.criterion-manual-dialog[open]').first();
  132 |     await expect(dialog).toBeVisible({ timeout: 10000 });
